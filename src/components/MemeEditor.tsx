...
const MemeEditor = ({ 
  ...
  if (!selectedTemplate && !customImage) {
    return (
      <div className="flex flex-col p-6 bg-gradient-to-br from-card to-card/80 rounded-lg text-center border border-meme-purple/10 shadow-md">
        <p className="text-lg font-medium bg-gradient-to-r from-meme-purple to-meme-pink bg-clip-text text-transparent">
          Please select a meme template or upload an image to start editing
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col ${compact ? "p-4" : "p-6"} bg-gradient-to-br from-card to-card/90 rounded-lg border border-meme-purple/20 shadow-lg`}>
      <div className="flex items-center mb-4">
        <h2 className={`${compact ? "text-lg" : "text-xl"} font-bold bg-gradient-to-r from-meme-blue to-meme-purple bg-clip-text text-transparent`}>
          Edit Your Meme
        </h2>
        <Sparkles size={compact ? 14 : 16} className="ml-2 text-meme-orange" />
      </div>
